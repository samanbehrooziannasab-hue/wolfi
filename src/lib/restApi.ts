...
const ROUTES: Record<string, Spec> = {
  ...
  "markets:listAllMarkets": { m: "GET", p: "/admin/workspace", pick: (d) => d?.markets ?? firstArray(d) },
  ...
  "admin:userDetail": { m: "GET", p: (a) => `/admin/users/${encodeURIComponent(String(a?.userId ?? a?.id ?? ""))}`, pick: (d) => d?.user ?? d },
