...
export default function SelfHostedPanel() {
  ...
  const mktsList = mkts.data?.markets ?? [];
  const fxCount = mktsList.filter((m: any) => m.market === "forex").length;
  const crCount = mktsList.length - fxCount;
  const openPosList = staff ? (adminOv.data?.openPositions ?? []) : (dash.data?.openPositions ?? []);
  const signalList = dash.data?.signals ?? [];
  const allStrats = staff ? (adminWs.data?.strategies ?? []) : (strats.data?.strategies ?? []);
  const activeStrats = allStrats.filter((st: any) => st.enabled !== false);
            ...
            {!staff ? (
              ...
              <Tabs value={adminTab} onValueChange={setAdminTab} className="space-y-4">
                ...
                <TabsContent value="users" className="space-y-4">
                  ...
                  <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm"><Users className="me-1 inline size-4 text-emerald-400" /> {s.tabUsers}</CardTitle></CardHeader><CardContent className="space-y-1">{(adminWs.data?.users ?? []).slice(0, 100).map((u: any) => (<div key={u.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/50 bg-background/40 px-3 py-2 text-[11px]"><span className="flex items-center gap-2"><span className="terminal-font font-bold" dir="ltr">{u.username ?? u.tg_id ?? u.id}</span><span className="text-muted-foreground">{u.name}</span>{u.is_vip && <Badge className="text-[9px] bg-amber-400/10 text-amber-300">{u.vip_package ?? "VIP"}</Badge>}{u.is_admin && <Badge className="text-[9px] bg-emerald-400/10 text-emerald-300">ADMIN</Badge>}{u.is_assistant && !u.is_admin && <Badge className="text-[9px] bg-blue-400/10 text-blue-300">ASSISTANT</Badge>}</span>{isAdmin && <div className="flex gap-1"><Switch checked={u.enabled} onCheckedChange={(v) => toggleUser(u.id, "enabled", v)} /><Select value={u.role ?? "user"} onValueChange={(v) => toggleUser(u.id, "role", v)}><SelectTrigger className="h-6 w-20 text-[9px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="user">User</SelectItem><SelectItem value="vip">VIP</SelectItem><SelectItem value="assistant">Assist</SelectItem><SelectItem value="admin">Admin</SelectItem></SelectContent></Select></div>}</div>))}{(adminWs.data?.users ?? []).length === 0 && <p className="py-6 text-center text-xs text-muted-foreground">{s.none}</p>}</CardContent></Card>
                </TabsContent>

                {/* ── admin wallet ────────────────────────────────────── */}
                <TabsContent value="wallet" className="space-y-4">
                  <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm"><CreditCard className="me-1 inline size-4 text-emerald-400" /> {s.recentTx}</CardTitle></CardHeader><CardContent className="space-y-1.5">{(adminWs.data?.transactions ?? []).length === 0 && <p className="py-6 text-center text-xs text-muted-foreground">{s.none}</p>}{(adminWs.data?.transactions ?? []).slice(0, 50).map((t: any) => (<div key={t.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/50 bg-background/40 px-3 py-1.5 text-[11px]"><span className="flex items-center gap-2"><Badge variant="outline" className="text-[9px]" dir="ltr">{t.type}</Badge><span className="text-muted-foreground">{t.username ?? "—"}</span><span className={`rounded px-1.5 py-0.5 text-[9px] font-bold ${t.status === "confirmed" || t.status === "done" ? "bg-emerald-400/10 text-emerald-300" : "bg-amber-400/10 text-amber-300"}`}>{t.status}</span></span><span className="terminal-font tabular-nums" dir="ltr">{fmtNum(t.amount, 4)} {t.asset ?? "USDT"}</span>{isAdmin && t.status === "pending" && <div className="flex gap-1"><Button size="sm" variant="outline" className="h-6 text-[9px] text-emerald-300" onClick={() => confirmTxn(t.id, true)}>✓</Button><Button size="sm" variant="outline" className="h-6 text-[9px] text-red-300" onClick={() => confirmTxn(t.id, false)}>✕</Button></div>}</div>))}</CardContent></Card>
                </TabsContent>

                {/* ── admin VIP ───────────────────────────────────────── */}
                <TabsContent value="vip" className="space-y-4">
                  <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm"><Trophy className="me-1 inline size-4 text-emerald-400" /> {s.adminVipMgt}</CardTitle></CardHeader><CardContent className="space-y-1.5">{(adminWs.data?.vipRequests ?? []).length === 0 && <p className="py-4 text-center text-xs text-muted-foreground">{s.none}</p>}{(adminWs.data?.vipRequests ?? []).slice(0, 30).map((r: any) => (<div key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/50 bg-background/40 px-3 py-2 text-[11px]"><span><span className="font-bold">{r.user_name ?? r.user_id}</span><span className="terminal-font ms-2" dir="ltr">{r.package_key}</span><span className="terminal-font ms-2 tabular-nums" dir="ltr">{fmtUsd(r.capital)}</span></span>{isAdmin && r.status === "pending" && <div className="flex gap-1"><Button size="sm" variant="outline" className="h-6 text-[9px] text-emerald-300" disabled={busy === `vr:${r.id}`} onClick={() => reviewVip(r.id, true)}>✓</Button><Button size="sm" variant="outline" className="h-6 text-[9px] text-red-300" disabled={busy === `vr:${r.id}`} onClick={() => reviewVip(r.id, false)}>✕</Button></div>}</div>))}</CardContent></Card>
                </TabsContent>
                ...
                <TabsContent value="coins" className="space-y-4">
                  ...
                  <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm">{s.coinLedger}</CardTitle></CardHeader><CardContent className="space-y-1.5">{(adminCoins.data?.ledger ?? []).slice(0, 60).map((r: any) => (<div key={r.id} className="flex justify-between rounded border border-border/40 bg-background/40 p-2 text-[10px]"><span>{r.username ?? r.user_id} · {r.reason}</span><span className="terminal-font" dir="ltr">{Number(r.delta) > 0 ? "+" : ""}{fmtNum(r.delta, 2)} {r.currency}</span></div>))}{(adminCoins.data?.ledger ?? []).length === 0 && <p className="py-6 text-center text-xs text-muted-foreground">{s.none}</p>}</CardContent></Card>
                </TabsContent>
                ...
                <TabsContent value="telegram">
                  <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm"><Send className="me-1 inline size-4 text-emerald-400" /> {s.adminTelegram}</CardTitle></CardHeader><CardContent className="space-y-3">
                    ...
                    {(adminWs.data?.telegram ?? []).slice(0, 30).map((tm: any) => (<div key={tm.id} className="rounded border border-border/40 bg-background/40 p-1.5 text-[10px]"><span className="font-bold">{tm.chat_id ?? tm.channel}</span><span className="ms-2 text-muted-foreground">{tm.text?.slice(0, 80)}</span><span className="ms-2 text-muted-foreground">{fmtTime(tm.created_at, lang)}</span></div>))}
                  </CardContent></Card>
                </TabsContent>
