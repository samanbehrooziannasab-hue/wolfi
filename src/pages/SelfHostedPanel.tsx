...
export default function SelfHostedPanel() {
  ...
  const openTicket = async (t: any) => { setSelTicket(t); setTicketMsgs([]); try { const r: any = await restFetch(`/api/support/tickets/${t.id}`, { token }); setTicketMsgs(asArr(r?.messages)); } catch {} };
  ...
  useEffect(() => { if (!token) { setPredHist([]); return; } restFetch("/api/coins/predictions", { token }).then((r: any) => setPredHist(asArr(r?.predictions))).catch(() => {}); }, [token, predGame]);
  useEffect(() => { if (!token) return; restFetch("/api/coins/packages", { token }).then((r: any) => setCoinPkgs(asArr(r?.packages))).catch(() => {}); }, [token]);
  ...
  const sigRows = myOnly ? asArr(mySigs.data?.signals) : asArr(sigs.data?.signals);
        ...
        <Tabs value={tab} onValueChange={setTab} className="space-y-4">
          ...
          <TabsContent value="overview" className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Card className="border-border/70 bg-card/60"><CardContent className="p-4"><p className="text-[11px] font-bold text-muted-foreground">{s.winRate}</p><p className="terminal-font mt-1 text-2xl font-black tabular-nums" dir="ltr">{staff ? `${ovStats?.winRate ?? 0}%` : "—"}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{staff ? Number(ovStats?.closedCount ?? 0) : asArr(dash.data?.closedPositions).length} {s.closedTrades}</p></CardContent></Card>
          ...
          <TabsContent value="monitor" className="space-y-4">
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm"><Landmark className="me-1 inline size-4 text-emerald-400" /> {s.adminExchanges}</CardTitle></CardHeader><CardContent className="space-y-1">{asArr(adminEx.data?.exchanges).length === 0 && <p className="py-4 text-center text-xs text-muted-foreground">{s.none}</p>}{asArr(adminEx.data?.exchanges).map((ex: any) => (<div key={ex.id} className="flex items-center justify-between rounded border border-border/40 bg-background/40 p-1.5 text-[10px]"><span className="terminal-font font-bold" dir="ltr">{ex.provider}</span><StatusBadge ok={ex.status === "connected"} okText="CONNECTED" badText={ex.status ?? s.offline} /></div>))}</CardContent></Card>
              <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm"><Bot className="me-1 inline size-4 text-emerald-400" /> {s.adminAiProviders}</CardTitle></CardHeader><CardContent className="space-y-1">{asArr(adminAi.data?.providers).length === 0 && <p className="py-4 text-center text-xs text-muted-foreground">{s.none}</p>}{asArr(adminAi.data?.providers).map((ai: any) => (<div key={ai.id} className="flex items-center justify-between rounded border border-border/40 bg-background/40 p-1.5 text-[10px]"><span className="terminal-font font-bold" dir="ltr">{ai.provider}</span><span className="text-muted-foreground" dir="ltr">{ai.model}</span><StatusBadge ok={ai.enabled} /></div>))}</CardContent></Card>
            </div>
            <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm"><Radio className="me-1 inline size-4 text-emerald-400" /> {s.adminLogs}</CardTitle></CardHeader><CardContent className="max-h-64 space-y-1 overflow-auto">{asArr(adminLog.data?.logs).slice(0, 30).length === 0 && <p className="py-4 text-center text-xs text-muted-foreground">{s.none}</p>}{asArr(adminLog.data?.logs).slice(0, 30).map((l: any) => (<div key={l.id} className="rounded border-b border-border/40 px-2 py-1 text-[10px]"><span className={`me-1.5 rounded px-1 py-0.5 font-bold ${String(l.level ?? "").includes("ERROR") || String(l.level ?? "").includes("WARN") ? "bg-red-400/10 text-red-300" : "bg-emerald-400/10 text-emerald-300"}`} dir="ltr">{l.level}</span><span className="terminal-font text-muted-foreground" dir="ltr">{fmtTime(l.created_at, lang)}</span><span className="ms-2">{l.message}</span></div>))}</CardContent></Card>
          ...
          <TabsContent value="education">
            ...
            <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm">{s.tabEducation}</CardTitle></CardHeader><CardContent className="space-y-2">{asArr(edu.data?.education).map((l: any) => (<article key={l.id} className="rounded-md border border-border/50 bg-background/40 p-3 text-[11px]"><h3 className="font-bold">{lang === "fa" ? l.title_fa : l.title_en}</h3><p className="mt-1 whitespace-pre-wrap leading-5 text-muted-foreground">{lang === "fa" ? l.body_fa : l.body_en}</p><p className="mt-2 text-[9px] text-muted-foreground">{fmtTime(l.created_at, lang)}</p></article>))}{asArr(edu.data?.education).length === 0 && <p className="py-8 text-center text-xs text-muted-foreground">{edu.loading ? s.loading : s.none}</p>}</CardContent></Card>
          </TabsContent>
...
<Card className="border-border/70 bg-card/60 lg:col-span-2"><CardHeader className="pb-2"><CardTitle className="text-sm"><BarChart3 className="me-1 inline size-4 text-emerald-400" /> {lang === "fa" ? "بازی پیش‌بینی قیمت (ساعتی)" : "Hourly price prediction game"}</CardTitle></CardHeader><CardContent className="space-y-2">
  ...
  <Card className="border-border/70 bg-card/60 lg:col-span-2"><CardHeader className="pb-2"><CardTitle className="text-sm"><Coins className="me-1 inline size-4" /> {lang === "fa" ? "پکیج‌های سکه" : "Coin packages"} <span className="text-[9px] font-normal text-muted-foreground">{lang === "fa" ? "(خرید با تومان)" : "(with toman)"}</span></CardTitle></CardHeader><CardContent className="space-y-1.5">{asArr(coinPkgs).map((pkg: any, pi: number) => (<div key={pi} className="flex flex-wrap items-center justify-between gap-2 rounded border border-border/40 bg-background/40 px-2 py-1.5 text-[10px]"><span>{lang === "fa" ? pkg.labelFa ?? pkg.label : pkg.label ?? pkg.labelFa} · <b className="text-emerald-300">{pkg.coins}</b> {lang === "fa" ? "سکه" : "coins"}</span><span className="terminal-font" dir="ltr">{fmtNum(pkg.price, 0)} {lang === "fa" ? "تومان" : "Toman"}</span><Button size="sm" variant="outline" className="h-6 text-[9px]" disabled={busy === `pkg:${pi}`} onClick={() => doBuyPackage(pi)}>{lang === "fa" ? "خرید" : "Buy"}</Button></div>))}{asArr(coinPkgs).length === 0 && <p className="text-[10px] text-muted-foreground">—</p>}</CardContent></Card>
<Card className="border-border/70 bg-card/60 lg:col-span-2"><CardHeader className="pb-2"><CardTitle className="text-sm">{s.coinLedger}</CardTitle></CardHeader><CardContent className="space-y-1.5">{asArr(coinData.data?.transactions).map((r: any) => (<div key={r.id} className="flex justify-between rounded border border-border/40 bg-background/40 p-2 text-[10px]"><span>{r.reason}</span><span className="terminal-font" dir="ltr">{Number(r.delta) > 0 ? "+" : ""}{fmtNum(r.delta, 2)} {r.currency}</span></div>))}</CardContent></Card>
            </div>
          </TabsContent>
      ...
      {unlocked[sig.id] && <div className="mt-1.5 rounded border border-emerald-400/20 bg-emerald-400/5 p-2 text-[10px]"><p className="flex flex-wrap gap-2" dir="ltr"><b>RR</b> {unlocked[sig.id].rr} · <b>conf</b> {unlocked[sig.id].confidence} · <b>TP1</b> {fmtNum(unlocked[sig.id].takeProfit, 5)}</p>{Array.isArray(unlocked[sig.id].reasonsFa) && unlocked[sig.id].reasonsFa.length > 0 && <ul className="mt-1 list-disc ps-3 text-muted-foreground">{unlocked[sig.id].reasonsFa.slice(0, 4).map((r: string, ri: number) => <li key={ri}>{r}</li>)}</ul>}</div>}
          ...
          <TabsContent value="notifications">
            <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><div className="flex items-center justify-between"><CardTitle className="text-sm"><Megaphone className="me-1 inline size-4 text-emerald-400" /> {s.tabNotifs}</CardTitle><Button size="sm" variant="outline" className="h-7 text-[10px]" onClick={markRead}>{s.markAllRead}</Button></div></CardHeader><CardContent className="space-y-1.5">{asArr(notifs.data?.notifications).length === 0 && <p className="py-6 text-center text-xs text-muted-foreground">{notifs.loading ? s.loading : s.none}</p>}{asArr(notifs.data?.notifications).map((n: any) => (<div key={n.id} className={`rounded-md border border-border/50 p-2.5 text-[11px] ${n.seen ? "bg-background/30 opacity-60" : "bg-background/40"}`}><div className="flex items-center justify-between gap-2"><span className="font-bold">{lang === "fa" ? n.title_fa ?? n.title : n.title_en ?? n.title}</span>{n.type && <Badge variant="outline" className="text-[9px]" dir="ltr">{n.type}</Badge>}</div>{(n.text_fa ?? n.text) && <p className="mt-0.5 text-muted-foreground">{lang === "fa" ? n.text_fa ?? n.text : n.text_en ?? n.text}</p>}<p className="mt-0.5 text-[9px] text-muted-foreground">{fmtTime(n.created_at, lang)}</p></div>))}</CardContent></Card>
          </TabsContent>
              ...
              <Tabs value={adminTab} onValueChange={setAdminTab} className="space-y-4">
                ...
                <TabsContent value="overview" className="space-y-4">
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <Stat label={s.openPositions} value={String(asArr(adminOv.data?.openPositions).length)} />
                  </div>
                  <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm">{s.openPositions}</CardTitle></CardHeader><CardContent className="space-y-1.5">{asArr(adminOv.data?.openPositions).length === 0 && <p className="py-6 text-center text-xs text-muted-foreground">{s.none}</p>}{asArr(adminOv.data?.openPositions).map((p: any) => (<div key={p.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/50 bg-background/40 px-3 py-2 text-[11px]"><span className="flex items-center gap-2"><span className="terminal-font font-bold" dir="ltr">{p.symbol}</span><DirBadge dir={p.side} /></span><span className="terminal-font tabular-nums" dir="ltr">{fmtNum(p.entry)}→{fmtNum(p.current)}</span><span className={`terminal-font font-bold tabular-nums ${Number(p.pnl ?? 0) >= 0 ? "text-emerald-300" : "text-red-300"}`} dir="ltr">{fmtUsd(p.pnl)}</span>{isAdmin && <><Button size="sm" variant="ghost" className="h-6 text-[10px]" disabled={busy === `ptg:${p.id}`} onClick={() => doPosTelegram(p.id)}><Send className="size-3" /></Button><Button size="sm" variant="ghost" className="h-6 text-[10px] text-red-400" onClick={() => closePos(p.id)}>✕</Button></>}</div>))}</CardContent></Card>
                </TabsContent>
                ...
                <TabsContent value="workspace" className="space-y-4">
                  <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm">{s.adminWorkspace}</CardTitle></CardHeader><CardContent className="space-y-2 text-[10px]">
                    <div className="grid gap-1"><span className="font-bold">{s.tabUsers}:</span> {asArr(adminWs.data?.users).length} کاربر · {asArr(adminWs.data?.positions).length} پوزیشن · {asArr(adminWs.data?.closed).length} بسته شده · {asArr(adminWs.data?.orders).length} سفارش</div>
                    <div className="grid gap-1"><span className="font-bold">{s.tabExchanges}:</span> {asArr(adminWs.data?.exchanges).length} صرافی · {asArr(adminWs.data?.providers).length} AI · {asArr(adminWs.data?.vipPackages).length} پکیج VIP</div>
                ...
                <TabsContent value="ai" className="space-y-4">
                  ...
                  <Card className="border-border/70 bg-card/60"><CardHeader className="pb-2"><CardTitle className="text-sm"><Bot className="me-1 inline size-4 text-emerald-400" /> {s.adminAiProviders}</CardTitle></CardHeader><CardContent className="space-y-1.5">{asArr(adminAi.data?.providers).length === 0 && <p className="py-4 text-center text-xs text-muted-foreground">{s.none}</p>}{asArr(adminAi.data?.providers).map((ai: any) => (<div key={ai.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/50 bg-background/40 px-3 py-2 text-[11px]"><span className="flex items-center gap-2"><span className="terminal-font font-bold" dir="ltr">{ai.provider}</span><span className="text-muted-foreground">{ai.model}</span><Badge variant="outline" className="text-[9px]">{ai.purpose ?? "general"}</Badge><StatusBadge ok={ai.enabled} /></span>{isAdmin && <div className="flex gap-1"><Button size="sm" variant="ghost" className="h-6 text-[10px]" disabled={busy.includes(`ait:${ai.id}`)} onClick={() => testAi(ai.id)}><Wifi className="size-3" /></Button><Switch checked={ai.enabled} onCheckedChange={(v) => toggleAi(ai.id, v)} /></div>}</div>))}
