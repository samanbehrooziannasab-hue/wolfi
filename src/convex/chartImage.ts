/**
 * chartImage.ts — server-side PNG candlestick chart renderer.
 *
 * Pure JS, zero dependencies (node:zlib for the PNG IDAT stream). Draws a
 * real candlestick chart from the market feed with a WOLF AI watermark and
...
// NOTE: pure module — the deflate function is injected so this file never
// imports Node builtins (Convex "use node" rule). Pass zlib.deflateSync from
// an action-only file, or from tests.
...
/** Encode an RGB (3 bytes per pixel) bitmap as a PNG. `deflate` is the zlib
 * deflate implementation (injected — pass zlib.deflateSync in Node actions). */
...
/** Uppercase only — used for chart labels (ASCII). */
...
class Canvas {
  ...
  /** Blend `c` into the background with `alpha` (0..1) — simulates transparency on RGB. */
...
export function renderCandleChartPng(
  ...
  const titleRight = textWidth(`${symbol}${tf ? " · " + tf : ""}`, 2) + 14;
  ...
  // Price range (include signal levels so SL/TP lines always fit)
  let lo = Infinity;
  let hi = -Infinity;
